(function($, window) {
    ...
    var tmplLabel = [
        '<div class="p-label" style="top:<%= top %>;left: <%= left %>;">',
        '   <p class="dot"></p>',
        '   <p class="name"><%= name %></p>',
        '</div>'
    ].join('');
    ...
    APPScroll.prototype.setCur = function(index) {
        $('.page-nav').text((index ) + '/' + this.options.images.length);
    };
    ...
    APPScroll.prototype.init = function() {
        ...
        this.scroll.on('scrollEnd', function(e) {
            var index = this.currentPage['pageX'] || 1;
            _self.setCur(index);
        });
    ...
    APPScroll.prototype.show = function() {
        ...
        $('.ui-scroll').show().addClass('animated fadeInUp');
    };
    ...
    return {
        init: function() {
            getRemoteData(imgId, function(data) {
                ...
                window.weixinData['title'] = window.weixinData['title'];
                window.weixinData['desc'] = data['atlas']['title'] || document.title;

                window.weixinData['imgUrl'] = data['image']['url'];

                var remark = data['atlas']['remark'];
                var remarkArr = eval(remark);

                for (var i = 0; i < remarkArr.length; i++) {
                    var item = remarkArr[i];
                    var name = item['name'];
                    var x = item['x'];
                    var y = item['y'];
                    var top = y * 100 + '%';
                    var left = x * 100 + '%';
                    var containerSize = getTargetSize($('#main')[0]);
                    var $item = $(_.template(tmplLabel)({top: top, left: left, name: name}));
                    $item.css('visibility', 'hidden');
                    $('#main').append($item);
                    (function($obj, delay) {
                        setTimeout(function(){
                            $obj.css('visibility', 'visible').addClass('animated bounceInDown');
                        }, delay);
                    })($item, Math.random() * 600);
                    //do bouds check;
                    if($item.width() + containerSize.w * parseFloat(x) > containerSize.w) {
                        $item.css({left: 'auto', right: (1-parseFloat(x)) * 100 + '%'});
                        $item.addClass('p-label-right');
                    }
                };
